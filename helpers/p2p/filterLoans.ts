...
export const convertLoanResultToLoanObj = (loan: any) => {
  return {
    ...
    createdAt: new BN(loan.account.createdAt).toString(),
    ...
    loanStartTime: new BN(loan.account.loanStartTime).toString(),
    ...
    paidBackAt: new BN(loan.account.paidBackAt).toString(),
    ...
    withdrewAt: new BN(loan.account.withdrewAt).toString()
  };
};
...
export const getDiscoverScreenLoanOrders = (loans: P2PLoans) => {
  ...
  let filteredOrders: P2PLoans = {};

  Object.keys(loans).map(loanPubkey => {
    if (!hasLender(loans[loanPubkey]))
